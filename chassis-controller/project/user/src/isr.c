/*********************************************************************************************************************
* MSPM0G3507 Opensource Library 即（MSPM0G3507 开源库）是一个基于官�?SDK 接口的第三方开源库
...
// 低优先级中断 - 5ms (按键扫描与蜂鸣器)
    ...
    key_scanner();    // 扫描按键物理状�?(极�?
    beep_actuator();  // 蜂鸣器递减倒计�?(极�?
...
{
// 高优先级中断 - 5ms (底盘控制与状态机计算)
    if (pit_callback_list[1] != NULL) {
    ...
    // 2. ⭐️ �?UI 状态机逻辑 (只做 if-else 数学计算，绝对不画屏幕，耗时几乎�?0)
    // menu_control();
...
void UART2_IRQHandler (void)
{
    switch(DL_UART_getPendingInterrupt(UART2))
    {
        ...
        case DL_UART_IIDX_RX:
        {
            // Task10 keeps the PC PID-tuning ASCII protocol.
            if (task_number == TASK_PID_OVAL_TRACK)
            {
                while(false == DL_UART_isRXFIFOEmpty(UART2))
                {
                    uint8_t rx_data = DL_UART_receiveData(UART2);
                    pid_tuning_rx_byte(rx_data);
                }
                break;
            }

            // All Raspberry Pi task protocols are intentionally disconnected.
            while(false == DL_UART_isRXFIFOEmpty(UART2))
            {
                (void)DL_UART_receiveData(UART2);
            }
        }break;
        default:    break;
    }
